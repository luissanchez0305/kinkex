<?php
	...
	class PYS_SOAPNamespaceList
	{
		...
		private function add1(PYS_SOAPNamespace $snSOAPNamespace)
		{
		...
		private function add2($szPrefix, $szSOAPNamespace)
		{
			...
			$this->m_lsnSOAPNamespaceList[] = new PYS_SOAPNamespace($szPrefix, $szSOAPNamespace);
		}
	...
	class PYS_SOAPParameter
	{
		private $m_szName;
	 	...
	 	private $m_lspaSOAPParamAttributeList;
	   	...
	   	{
			$this->m_szValue = PYS_SharedFunctions::replaceCharsInStringWithEntities($szValue);
	   	}
	   	...
	   	//constructor
	   	public function __construct($szName, $szValue, PYS_SOAPParamAttributeList $lspaSOAPParamAttributeList = null)
	   	{
	   		$nCount = 0;
	   		...
	   		$this->m_lspSOAPParamList = new PYS_SOAPParamList();
			$this->m_lspaSOAPParamAttributeList = new PYS_SOAPParamAttributeList();
	   		
	   		if ($lspaSOAPParamAttributeList != null)
	   		{
	   			for ($nCount = 0; $nCount < $lspaSOAPParamAttributeList->getCount();$nCount++)
	   			{
	   				$spaSOAPParamAttribute = new PYS_SOAPParamAttribute($lspaSOAPParamAttributeList->getAt($nCount)->getName(), $lspaSOAPParamAttributeList->getAt($nCount)->getValue());
	   	...
	   	function toXMLString()
	   	{
	   		...
	   		if ($this->m_lspaSOAPParamAttributeList != null)
	   		{
	   			for ($nCount = 0; $nCount < $this->m_lspaSOAPParamAttributeList->getCount(); $nCount++)
	   			{
		   			...
		   			{
		   				$sbString = '';
		   				$sbString .= ' ' .$spaAttribute->getName(). '="' .PYS_SharedFunctions::replaceCharsInStringWithEntities($spaAttribute->getValue()). '"';
	   		...
	   		else
	   		{
	   			...
	   			if ($this->getValue() != '')
	   			{
	   				$sbReturnString .= PYS_SharedFunctions::replaceCharsInStringWithEntities($this->getValue());
	   			}
	...
	class PYS_SOAPParamList
	{
		...
		protected function add1(PYS_SOAPParameter $spSOAPParam)
		{
		...
		protected function add2($szName, $szValue)
		{
			...
			if ($szName != '' &&
				$szName != null)
			{
				$this->m_lspSOAPParamList[] = new PYS_SOAPParameter($szName, $szValue);
			}
	...
	class PYS_SOAPParamAttributeList
	{
		...
		private function add1(PYS_SOAPParamAttribute $spaSOAPParamAttributeAttribute)
		{
		...
		private function add2($szName, $szValue)
		{
			...
			if ($szName != '' &&
				$szName != null)
			{
				$nReturnValue = array_push($this->m_lspaSOAPParamAttributeAttributeList, new PYS_SOAPParamAttribute($szName, $szValue));
			}
	...
	class PYS_SOAP
	{
	    ...
	    public function addParam1($szName, $szValue, PYS_SOAPParamAttributeList $lspaSOAPParamAttributeList = null)
	    {
	    	$spSOAPParam;
	    	
	    	$spSOAPParam = new PYS_SOAPParameter($szName, $szValue, $lspaSOAPParamAttributeList);
	    	
	    	$this->addParam2($spSOAPParam, true);
	    }
	    public function addParam2(PYS_SOAPParameter $spSOAPParam, $boOverWriteValue)
	    {
	        ...
	        $lszHierarchicalNames = new PYS_StringList();
	        
	        $lszHierarchicalNames = PYS_SharedFunctions::getStringListFromCharSeparatedString($spSOAPParam->getName(), '.');
	        ...
	        else 
	        {
	        	...
	        	for ($nCount = 0; $nCount < $lszHierarchicalNames->getCount(); $nCount++)
	        	{
	        		// get the current tag name
	              	...
	              	//continuework
	               	$szTagNameToFind = PYS_SharedFunctions::getArrayNameAndIndex($szString, $nCurrentIndex);
	             		...
	             		$nCount == 0)
	             	{
	                    ...
	                    $spWorkingSOAPParam = PYS_Functions::isSOAPParamInParamList($lspParamList, $szTagNameToFind, $nCurrentIndex);
	                        ...
	                        if ($nCount == ($lszHierarchicalNames->getCount() - 1))
	                        {
	                            ...
	                            for ($nCount2 = 0; $nCount2 < $spSOAPParam->getSOAPParamAttributeList()->getCount(); $nCount2++)
	                            {
	                                ...
	                                $spaAttribute = $spSOAPParam->getSOAPParamAttributeList()->getAt($nCount2);
	                              	...
	                              	if ($spaAttribute != null)
	                                {
	                                	$spaNewAttribute = new PYS_SOAPParamAttribute($spaAttribute->getName(), $spaAttribute->getValue());
	             	...
	             	if (!$boFound)
	                {
	                    ...
	                    if ($nCount == ($lszHierarchicalNames->getCount() - 1))
	                    {
	                    	$lspaAttributeList = new PYS_SOAPParamAttributeList();
	                        ...
	                        $spNewSOAPParam = new PYS_SOAPParameter($szTagNameToFind, $spSOAPParam->getValue(), $lspaAttributeList);

	                        $lspParamList->add($spNewSOAPParam);
	                 	}
	                    else
	                    {
	                    	$spNewSOAPParam = new PYS_SOAPParameter($szTagNameToFind, '', null);
	                        $lspParamList->add($spNewSOAPParam);
	    ...
	    private function addParamAttribute1($szName, $szParamAttributeName, $szParamAttributeValue)
	    {
	    	...
	    	$lspaSOAPParamAttributeList = new PYS_SOAPParamAttributeList();
	    	$spaSOAPParamAttribute = new PYS_SOAPParamAttribute($szParamAttributeName, $szParamAttributeValue);
	    	$lspaSOAPParamAttributeList->add($spaSOAPParamAttribute);
	    	
	    	$spSOAPParam = new PYS_SOAPParameter($szName, '', $lspaSOAPParamAttributeList);
	    	
	    	$this->addParam2($spSOAPParam, false);
	    }
	    private function addParamAttribute2($szName, PYS_SOAPParamAttribute $spaSOAPParamAttribute)
	    {
	    	$spSOAPParam;
	    	$lspaSOAPParamAttributeList;
	    ...
	    private function SOAP3($szMethod, $szMethodURI, $szURL, $szSOAPEncoding, $boAddDefaultNamespaces, PYS_SOAPNamespaceList $lsnSOAPNamespaceList = null)
	    {
	    	$snSOAPNamespace;
	       	$nCount = 0;
	        ...
	        $this->m_lsnSOAPNamespaceList = new PYS_SOAPNamespaceList();

	      	if ($boAddDefaultNamespaces)
	        {
	        	$snSOAPNamespace = new PYS_SOAPNamespace('soap', 'http://schemas.xmlsoap.org/soap/envelope/');
	          	$this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	            $snSOAPNamespace = new PYS_SOAPNamespace('xsi', 'http://www.w3.org/2001/XMLSchema-instance');
	            $this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	           	$snSOAPNamespace = new PYS_SOAPNamespace('xsd', 'http://www.w3.org/2001/XMLSchema');
	            $this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	        }
	        if ($lsnSOAPNamespaceList != null)
	      	{
	            ...
	            {
	             	$snSOAPNamespace = new PYS_SOAPNamespace($lsnSOAPNamespaceList->getAt($nCount)->getPrefix(), $lsnSOAPNamespaceList->getAt($nCount)->getNamespace());
	              	$this->m_lsnSOAPNamespaceList->add($snSOAPNamespace);
	            }
	        }
	        $this->m_lspSOAPParamList = new PYS_SOAPParamList();
	...
	class PYS_Functions
	{
		public static function isSOAPParamInParamList(PYS_SOAPParamList $lspParamList, $szTagNameToFind, $nIndex)
		{
			$spReturnParam = null;
			$boFound = false;
			$nFound = 0;
