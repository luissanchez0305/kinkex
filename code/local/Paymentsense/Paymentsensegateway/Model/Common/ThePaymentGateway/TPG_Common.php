<?php
	//XMLEntities
	$g_XMLEntities = array();
	$g_XMLEntities[] = new PYS_XMLEntity(0x26, "&amp;");
	$g_XMLEntities[] = new PYS_XMLEntity(0x22, "&quot;");
	$g_XMLEntities[] = new PYS_XMLEntity(0x27, "&apos;");
	$g_XMLEntities[] = new PYS_XMLEntity(0x3c, "&lt;");
	$g_XMLEntities[] = new PYS_XMLEntity(0x3e, "&gt;");

	abstract class PYS_Nullable
	{
	...
	class PYS_NullableInt extends PYS_Nullable 
	{
		private $m_nValue;
	...
	class PYS_NullableBool extends PYS_Nullable 
	{
		private $m_boValue;
	...
	class PYS_ISOCountryList
	{
		...
		public function getISOCountry($szCountryShort, &$icISOCountry)
		{
					...
					$nCount < count($this->m_licISOCountries))
			{
			    ...
			    {
			    	$icISOCountry = new PYS_ISOCountry($icISOCountry2->getISOCode(), $icISOCountry2->getCountryName(), $icISOCountry2->getCountryShort(), $icISOCountry2->getListPriority());
			    	$boFound = true;
			    }
		...
		{
			$newISOCountry = new PYS_ISOCountry($nISOCode, $szCountryName, $szCountryShort, $nListPriority);
	...
	class PYS_ISOCurrencyList
	{
		...
		public function getISOCurrency($szCurrencyShort, &$icISOCurrency)
		{
	        ...
	        while (!$boFound &&
	              	$nCount < count($this->m_licISOCurrencies))
	     	{
	            ...
	            {
	            	$icISOCurrency = new PYS_ISOCurrency($icISOCurrency2->getISOCode(), $icISOCurrency2->getCurrency(),$icISOCurrency2->getCurrencyShort(), $icISOCurrency2->getExponent());
	                $boFound = true;
	          	}
		...
		{
			$newISOCurrency = new PYS_ISOCurrency($nISOCode, $szCurrency, $szCurrencyShort, $nExponent);
	...
	class PYS_SharedFunctions
	{
		public static function getNamedTagInTagList($szName, $xtlTagList)
		{
	        ...
	        $lszHierarchicalNames = PYS_SharedFunctions::getStringListFromCharSeparatedString($szName, '.');
		...
		public static function getStringListFromCharSeparatedString($szString, $cDelimiter)
		{
	        ...
	        if ($szString == null ||
	      	...
	      	$lszStringList = new PYS_StringList();
		...
		public static function getValue($szXMLVariable, $xtlTagList, & $szValue)
		{
			...
			if (xtlTagList == null)
	       	...
	       	$lszHierarchicalNames = new PYS_StringList();
	        $szValue = null;
	        $lszHierarchicalNames = PYS_SharedFunctions::getStringListFromCharSeparatedString($szXMLVariable, '.');
	        
			if (count($lszHierarchicalNames) == 1)
	     	{
	       		$szXMLTagName = $lszHierarchicalNames->getAt(0);

	            $xtCurrentTag = PYS_SharedFunctions::GetNamedTagInTagList($szXMLTagName, $xtlTagList);
	    		...
	    		if (count($lszHierarchicalNames) > 1)
	          	{
	                ...
	                if ($xtCurrentTag != null)
	                {
	                    ...
	                    else
	                    {
	                        ...
	                        $xtCurrentTag = PYS_SharedFunctions::getNamedTagInTagList($szLastXMLTagName, $xtCurrentTag->getChildTags());

	                        if ($xtCurrentTag != null)
	                        {
	                        	$szValue = PYS_SharedFunctions::replaceEntitiesInStringWithChars($xtCurrentTag->getContent());
	                            $boReturnValue = true;
	                      	}
	                   	}
	             	}
