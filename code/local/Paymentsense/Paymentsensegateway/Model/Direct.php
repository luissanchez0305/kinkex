...
class Paymentsense_Paymentsensegateway_Model_Direct extends Mage_Payment_Model_Method_Abstract
{
	/**
  	...
  	* @var string [a-z0-9_]
  	*/
	protected $_code = 'paymentsensegateway';
 	protected $_formBlockType = 'paymentsensegateway/form'; 
 	protected $_infoBlockType = 'paymentsensegateway/info';
	...
	*/  
	...
	public function _runTransaction(Varien_Object $payment, $amount)
	{
		...
		$iclISOCountryList = PYS_ISOCountries::getISOCountryList();
		...
		$cdtCardDetailsTransaction = new PYS_CardDetailsTransaction($rgeplRequestGatewayEntryPointList);
		...
		if ($boTransactionProcessed == false)
		{
			...
			$error = Paymentsense_Paymentsensegateway_Model_Common_GlobalErrors::ERROR_261;
	...
	public function _runHostedPaymentTransaction(Varien_Object $payment, $amount)
	{
		...
		{
			case Paymentsense_Paymentsensegateway_Model_Source_ResultDeliveryMethod::RESULT_DELIVERY_METHOD_POST:
				$szCallbackURL = Mage::getUrl('paymentsensegateway/payment/callbackhostedpayment', array('_secure' => true));
				break;
			case Paymentsense_Paymentsensegateway_Model_Source_ResultDeliveryMethod::RESULT_DELIVERY_METHOD_SERVER:
				$szCallbackURL = Mage::getUrl('paymentsensegateway/payment/callbackhostedpayment', array('_secure' => true));
				$szServerResultURL = Mage::getUrl('paymentsensegateway/payment/serverresult', array('_secure' => true));
				$boPaymentFormDisplaysResult = 'true';
				break;
			case Paymentsense_Paymentsensegateway_Model_Source_ResultDeliveryMethod::RESULT_DELIVERY_METHOD_SERVER_PULL:
				$szCallbackURL = Mage::getUrl('paymentsensegateway/payment/serverpullresult', array('_secure' => true));
				break;
		}
	...
	public function _runTransparentRedirectTransaction(Varien_Object $payment, $amount)
	{
		...
		$szHashDigest = PYS_PaymentFormHelper::calculateTransparentRedirectHashDigest($szMerchantID, $szPassword, $hmHashMethod, $szPreSharedKey, $nAmount, $nCurrencyCode, $szOrderID, $szTransactionType, $szTransactionDateTime, $szCallbackURL, $szOrderDescription);
	...
	public function _run3DSecureTransaction(Varien_Object $payment, $szPaRes, $szMD)
	{
		...
		$tdsaThreeDSecureAuthentication = new PYS_ThreeDSecureAuthentication($rgeplRequestGatewayEntryPointList);
		...
		{
			// could not communicate with the payment gateway
			$szLogMessage = Paymentsense_Paymentsensegateway_Model_Common_GlobalErrors::ERROR_431;
			$message = Paymentsense_Paymentsensegateway_Model_Common_GlobalErrors::ERROR_431;
			Mage::log($szLogMessage);
		}
		else
		{
		...
		if($error == true)
		{
			$message = Mage::helper('paymentsensegateway')->__($message);
	...
	*/
     ...
     * @return unknown
     */
    public function pysCollection(Varien_Object $payment, $szOrderID, $szCrossReference)
    {
