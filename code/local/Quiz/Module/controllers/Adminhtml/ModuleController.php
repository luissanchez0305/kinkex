...
class Quiz_Module_Adminhtml_ModuleController extends Mage_Adminhtml_Controller_Action
{
			...
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				try {	
					...
					// Any extention would work
	           		$uploader->setAllowedExtensions(array('mp4','flv','mpeg','mkv'));
					...
					$path = Mage::getBaseDir() . DS . 'video' .DS ;
        ...
        if(!is_array($moduleIds)) {
            ...
            try {
                foreach ($moduleIds as $moduleId) {
                    $module = Mage::getModel('module/module')->load($moduleId);
                    $module->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($moduleIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
        ...
        if(!is_array($moduleIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($moduleIds))
                );
            } catch (Exception $e) {
    ...
    {
        $fileName   = 'module.csv';
        $content    = $this->getLayout()->createBlock('module/adminhtml_module_grid')
            ->getCsv();
